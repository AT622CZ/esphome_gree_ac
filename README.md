...
| `i_feel_sensor` | id of a `sensor` | - | **experimental**: send the sensor's value to the unit as the I FEEL temperature, the way the IR remote does. Sets the I FEEL flag (data byte 9 bit6) and the temperature (data byte 24, whole °C) in every SET packet; if the unit honours it, it regulates by that temperature and reports it back as room temperature. An update packet is sent whenever the sensor changes by a whole degree and every 10 minutes (like the remote). Watch the log line `Unit reports I FEEL active, I FEEL temperature ..` to see whether the unit accepted it. |
| `i_feel_interval` | time, min `30s` | `5min` | **experimental**: how often the I FEEL temperature is resent so the unit keeps trusting it (the IR remote resends every 10 minutes; the unit's own timeout is unknown, so the default is shorter). |
| `i_feel_command` | `true`, `false` | `false` | **experimental**: `false` sends the I FEEL update as a plain packet (no 0xAF, no "no change" flag), `true` as a full command with 0xAF (the unit may beep on each update). Try `true` if the unit ignores the plain packets. |
